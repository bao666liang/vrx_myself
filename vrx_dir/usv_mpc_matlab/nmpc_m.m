function [t, x, u] = nmpc_m(runningcosts, terminalcosts, ...
       ...
       trecord = trecord + t_Elapsed;
        ...
        if ( iprint >= 1 )
            printSolution(system, printHeader, printClosedloopData, ...
                          ...
                          exitflag, output, t_Elapsed, trecord);
        end
        ...
        mpciter = mpciter+1;
        yref_iter = shiftRef(yref_iter,yref,mpciter,N);
...
function [u, V, exitflag, output] = solveOptimalControlProblem ...
    ...
    linearconstraints, system, N, t0, x0, u0, T, yref_iter,...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)
    ...
    for k=1:N
        [Anew, bnew, Aeqnew, beqnew, lbnew, ubnew] = ...
               linearconstraints(t0+k*T,x(k,:),u0(:,k)); %same for all the loops; ...
           %can be changed for varying constraints
        A = blkdiag(A,Anew);
        b = [b, bnew];
    ...
    [u, V, exitflag, output] = fmincon(@(u) costfunction(runningcosts, ...
        ...
        u, yref_iter, atol_ode_sim, rtol_ode_sim, type), u0, A, b, Aeq, beq, lb, ...
...
function cost = costfunction(runningcosts, terminalcosts, system, ...
    ...
    x = computeOpenloopSolution(system, N, T, t0, x0, u, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x(k,:), yref_iter(k,:), u(:,k));
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x(N+1,:), yref_iter(k,:), u(:,k));
end
...
function printSolution(system, printHeader, printClosedloopData, ...
             ...
             atol_ode, rtol_ode, type, iprint, exitflag, output, t_Elapsed, trecord)
    if (mpciter == 0)
        printHeader();
    end
    printClosedloopData(mpciter, u, x0, trecord);
    switch exitflag
        case -2
...
function yref_iter = shiftRef(yref_iter,yref,mpciter,N)
   yref_iter = [yref_iter(2:size(yref_iter,1),:); yref(mpciter+N,:)];
end
